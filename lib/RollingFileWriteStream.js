  ...
  try {
      fs.access(sourceFilePath, fs.constants.W_OK | fs.constants.R_OK, e => {
          ...
          if (needCompress) {
              ...
              const writerStream = readStream.pipe(zlib.createGzip()).pipe(fs.createWriteStream(targetFilePath));
              ...
              writerStream.on('error', function (err) {
                  ...
                  fs.unlink(sourceFilePath, (err, result) => {
                      return done();
                  });
              });
              /*
...
class RollingFileWriteStream extends Writable {
      ...
      cb(
        ...
        _.sortBy(
          ...
          n => {
              if (n.date) {
                  try {
                      return format.parse(options.pattern, n.date).valueOf() - n.index;
                  } catch (error) {
                      return newNow().valueOf() - n.index;
                  }
              }
              else {
                  return newNow().valueOf() - n.index;
  ...
  //重写end方法，解决在调用log4js.shutdown()方法是没有关闭FD问题
  end() {
